...
export default function Reversed3DFoldTemplateFixed() {
  ...
  useEffect(() => {
    console.log('🚀 Initializing Fixed Reversed 3D Fold Template')
    ...
    setTimeout(() => {
      ...
      const TILE_TRANSITION_DISTANCE = 500  // Increased from 400px for smoother transitions
      const maxScroll = (tilesPerPanel - 1) * TILE_TRANSITION_DISTANCE
      ...
      const updateScroll = (position: number) => {
        panels.forEach((panel, panelIndex) => {
          ...
          if (content) {
            // Smoother tile switching with fade
            const tileOffset = position / TILE_TRANSITION_DISTANCE
            const currentTileIndex = Math.floor(tileOffset)
            const transitionProgress = tileOffset % 1
            
            const tiles = content.querySelectorAll('.tile-wrapper')
            tiles.forEach((tile, i) => {
              const tileElement = tile as HTMLElement
              const targetIndex = (currentTileIndex + panelIndex) % tilesPerPanel
              
              if (i === targetIndex) {
                tileElement.style.display = 'flex'
                tileElement.style.opacity = '1'
              } else if (i === (targetIndex + 1) % tilesPerPanel && transitionProgress > 0.5) {
                // Show next tile fading in
                tileElement.style.display = 'flex'
                tileElement.style.opacity = String((transitionProgress - 0.5) * 2)
              } else {
                tileElement.style.display = 'none'
              }
            })
            
            // Smoother position interpolation with reduced movement
            const innerOffset = (position % TILE_TRANSITION_DISTANCE) * 0.6
        ...
        setDebugInfo(prev => ({
          ...
          currentTile: Math.floor(position / TILE_TRANSITION_DISTANCE),
          visibleTiles: [0, 1, 2, 3].map(i => 
            (Math.floor(position / TILE_TRANSITION_DISTANCE) + i) % tilesPerPanel
          )
        }))
      }
      ...
      const animate = () => {
        ...
        if (Math.abs(diff) > 0.1) {
          currentScroll += diff * 0.12  // Smoother interpolation
      ...
      window.navigateToTile = (tileIndex: number) => {
        targetScroll = tileIndex * TILE_TRANSITION_DISTANCE
      }

      window.nextTile = () => {
        const currentTile = Math.floor(targetScroll / TILE_TRANSITION_DISTANCE)
        targetScroll = Math.min(maxScroll, (currentTile + 1) * TILE_TRANSITION_DISTANCE)
      }

      window.prevTile = () => {
        const currentTile = Math.floor(targetScroll / TILE_TRANSITION_DISTANCE)
        targetScroll = Math.max(0, (currentTile - 1) * TILE_TRANSITION_DISTANCE)
      }

      const handleWheel = (e: WheelEvent) => {
        e.preventDefault()
        targetScroll += e.deltaY * 0.3  // Reduced sensitivity for smoother scrolling
      }
  ...
  return (
    <div style={{ 
      ...
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #fold-wrapper-reversed {
          position: absolute;
          top: 50%;
          left: 55%;  /* Shifted right to show full content */
          transform: translate(-50%, -50%);
          width: 90%;  /* Wider to accommodate content */
          height: 65vh;
          perspective: 200vw;  /* Gentler perspective */
          transform-style: preserve-3d;
        }
        
        .fold-panel-reversed {
          position: absolute;
          width: 52vw;  /* Wider panels for better visibility */
          height: 65vh;
          overflow: hidden;
          top: 0;
          left: 0;
        ...
        /* Gentler fold angles for better visibility */
        ...
        .fold-panel-reversed-1 {
          ...
          transform: translate3d(95%, 0, -50px) rotateY(12deg);
          z-index: 3;
        }
        ...
        .fold-panel-reversed-2 {
          transform-origin: left center;
          transform: translate3d(95%, 0, -50px) rotateY(12deg) 
                     translate3d(95%, 0, -50px) rotateY(-8deg);
          z-index: 2;
        }
        ...
        .fold-panel-reversed-3 {
          ...
          transform: translate3d(95%, 0, -50px) rotateY(12deg) 
                     translate3d(95%, 0, -50px) rotateY(-8deg)
                     translate3d(95%, 0, -50px) rotateY(-20deg);
          z-index: 1;
          opacity: 0.92;
        }
        
        /* Subtle edge shadows for depth */
        .fold-panel-reversed::before {
          content: '';
          position: absolute;
          top: 0;
          left: 0;
        ...
        .fold-content-reversed {
          ...
          padding-right: 2rem;  /* Prevent edge clipping */
        }
      ...
      {/* 3D Fold Structure - REVERSED with FIXED angles */}
